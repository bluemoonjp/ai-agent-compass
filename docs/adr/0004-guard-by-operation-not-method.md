...
Editing issues, milestones, labels, and branch-protection settings through `gh api` no longer trips a deny rule, because none of the new rules are keyed on the HTTP method those edits happen to use. Merging, tagging, publishing a release, changing repository visibility, deleting the repository, and managing secrets remain blocked for an agent working through the guarded shell tools, whether reached directly or through an equivalent `gh api`/GraphQL call — the same mitigation ADR-0001 established, now harder to route around, still scoped to one cooperating agent.
...
- The rule guarding release-related `gh api` calls blocks reading releases, not only creating or deleting them, for the same reason. #24's planned release-check reads a GitHub Release; if it does that through `gh api`, its implementation needs to either read some other way or split the read path from this guard — a decision for #24, not this one.
- A tag push naming the ref directly, without a `--tags`/`--follow-tags` flag, is not matched by the push rule; it relies on tag creation itself being blocked by the tag-creation rule.
- The repository's tag protection does not cover tag creation, so the client-side rules blocking `git tag` and tag pushes are load-bearing, not redundant with server-side enforcement.
...
- `git tag *`'s trailing wildcard also matches the bare `git tag` (a read-only tag listing), not only `git tag <name>` (creation): Claude Code's permission-pattern language matches a bare command whenever a rule's only wildcard is a trailing one preceded by a space, and there is no way to write a rule that blocks tag creation without also blocking tag listing. This is accepted as a minor usability cost, not a security gap, and resolves the gap this ADR's Context section raised without fixing.
