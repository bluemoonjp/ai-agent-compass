...
It does not solve human review on a single-maintainer repository. GitHub cannot require a second human's approval when there is only one maintainer, so a required-reviewers rule is not available here. The Claude Code path is narrowed instead: `.claude/settings.json`'s `permissions.deny` blocks `gh pr merge`, `gh pr ready`, mutating `gh api` calls, secret and release management, and tag pushes, so an agent working through Claude Code cannot merge its own work or touch the repository's trust boundary unsupervised. This is a mitigation for one agent, not a substitute for review: it has no effect on any other agent or on a human running the same commands directly, and it is recorded here as a known limitation rather than a solved problem.
